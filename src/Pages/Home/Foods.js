import React from 'react';

const Foods = () => {
    return (
        <>
        <section className="_ftre_fd_dlvry2_sec">
			<div className="container">
				<div className="_ftre_fd_dlvry2_top">
					<h1 className="_ftre_fd_dlvry2_tit4">
                        Popular Menu              
                    </h1>
					<h1 className="_ftre_fd_dlvry2_titl">
						Wait a minute for delicious.
					</h1>
				</div>
				<div className="row">
					<div className="col-12 col-md-6 col-lg-6 col-xl-4">
						<div className="_ftre_fd_dlvry2_crd">
							<div className="_ftre_fd_dlvry2_pic">
								<img src="assets/img/ftre_fd_dlvry1.svg" alt="" />
							</div>
							<div className="_ftre_fd_dlvry2_inr">
								<ul className="_ftre_fd_dlvry2_ul">
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
								</ul>
								<p className="_ftre_fd_dlvry2_crd_titl">
									New York-Style pizza
								</p>
								<p className="_ftre_fd_dlvry2_crd_txt">
									Lorem Ipsum Lorem Ipsum is simply and dummy text of the printing.
								</p>
								<div className="_ftre_fd_dlvry2_btm">
									<div className="_ftre_fd_dlvry2_btm_lft">
										<p className="_ftre_fd_dlvry2_btm_txt">
											$7.10
										</p>
									</div>
									<div className="_ftre_fd_dlvry2_btm_r8">
										<button className='btn btn-info btn-sm'>Order Now</button>
										{/* <ul className="_ftre_fd_dlvry2_btm_ul">
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" fill="none" viewBox="0 0 16 14">
												  <path stroke="#fff" strokeWidth="1.5" d="M7.414 2.175L8 2.907l.585-.732c.062-.077.103-.13.135-.174.05-.065.078-.103.128-.153a3.75 3.75 0 015.31 5.296L8 12.968 1.84 7.144a3.75 3.75 0 015.31-5.296c.05.05.08.088.128.153l.135.174z"/>
												</svg>
											</li>
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 18 18">
													<path fill="#fff" d="M13.5 12.064c-.57 0-1.084.221-1.474.577L6.683 9.525c.04-.169.067-.345.067-.525 0-.18-.026-.356-.067-.525l5.287-3.086c.401.375.938.607 1.53.607a2.252 2.252 0 002.25-2.25 2.252 2.252 0 00-2.25-2.25 2.252 2.252 0 00-2.25 2.25c0 .18.026.357.068.525L6.03 7.358A2.233 2.233 0 004.5 6.75 2.252 2.252 0 002.25 9a2.252 2.252 0 002.25 2.25c.593 0 1.129-.232 1.53-.607l5.344 3.116c-.038.157-.06.322-.06.491a2.186 2.186 0 102.186-2.186z"/>
												</svg>
											</li>
										</ul> */}
									</div>
								</div>
							</div>
						</div>
					</div>
					<div className="col-12 col-md-6 col-lg-6 col-xl-4">
						<div className="_ftre_fd_dlvry2_crd">
							<div className="_ftre_fd_dlvry2_pic">
								<img src="assets/img/ftre_fd_dlvry2.svg" alt="" />
							</div>
							<div className="_ftre_fd_dlvry2_inr">
								<ul className="_ftre_fd_dlvry2_ul">
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
								</ul>
								<p className="_ftre_fd_dlvry2_crd_titl">
									Bento Ekiben Makunchi ..
								</p>
								<p className="_ftre_fd_dlvry2_crd_txt">
									Lorem Ipsum Lorem Ipsum is simply and dummy text of the printing.
								</p>
								<div className="_ftre_fd_dlvry2_btm">
									<div className="_ftre_fd_dlvry2_btm_lft">
										<p className="_ftre_fd_dlvry2_btm_txt">
											$6.40
										</p>
									</div>
									<div className="_ftre_fd_dlvry2_btm_r8">
										<ul className="_ftre_fd_dlvry2_btm_ul">
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" fill="none" viewBox="0 0 16 14">
												  <path stroke="#fff" strokeWidth="1.5" d="M7.414 2.175L8 2.907l.585-.732c.062-.077.103-.13.135-.174.05-.065.078-.103.128-.153a3.75 3.75 0 015.31 5.296L8 12.968 1.84 7.144a3.75 3.75 0 015.31-5.296c.05.05.08.088.128.153l.135.174z"/>
												</svg>
											</li>
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 18 18">
													<path fill="#fff" d="M13.5 12.064c-.57 0-1.084.221-1.474.577L6.683 9.525c.04-.169.067-.345.067-.525 0-.18-.026-.356-.067-.525l5.287-3.086c.401.375.938.607 1.53.607a2.252 2.252 0 002.25-2.25 2.252 2.252 0 00-2.25-2.25 2.252 2.252 0 00-2.25 2.25c0 .18.026.357.068.525L6.03 7.358A2.233 2.233 0 004.5 6.75 2.252 2.252 0 002.25 9a2.252 2.252 0 002.25 2.25c.593 0 1.129-.232 1.53-.607l5.344 3.116c-.038.157-.06.322-.06.491a2.186 2.186 0 102.186-2.186z"/>
												</svg>
											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div className="col-12 col-md-6 col-lg-6 col-xl-4">
						<div className="_ftre_fd_dlvry2_crd">
							<div className="_ftre_fd_dlvry2_pic">
								<img src="assets/img/ftre_fd_dlvry3.svg" alt="" />
							</div>
							<div className="_ftre_fd_dlvry2_inr">
								<ul className="_ftre_fd_dlvry2_ul">
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
								</ul>
								<p className="_ftre_fd_dlvry2_crd_titl">
									Humburger Veggei ..
								</p>
								<p className="_ftre_fd_dlvry2_crd_txt">
									Ut enim ad minim veniam, quis nostrud exercitation.
								</p>
								<div className="_ftre_fd_dlvry2_btm">
									<div className="_ftre_fd_dlvry2_btm_lft">
										<p className="_ftre_fd_dlvry2_btm_txt">
											$4.80
										</p>
									</div>
									<div className="_ftre_fd_dlvry2_btm_r8">
										<ul className="_ftre_fd_dlvry2_btm_ul">
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" fill="none" viewBox="0 0 16 14">
												  <path stroke="#fff" strokeWidth="1.5" d="M7.414 2.175L8 2.907l.585-.732c.062-.077.103-.13.135-.174.05-.065.078-.103.128-.153a3.75 3.75 0 015.31 5.296L8 12.968 1.84 7.144a3.75 3.75 0 015.31-5.296c.05.05.08.088.128.153l.135.174z"/>
												</svg>
											</li>
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 18 18">
													<path fill="#fff" d="M13.5 12.064c-.57 0-1.084.221-1.474.577L6.683 9.525c.04-.169.067-.345.067-.525 0-.18-.026-.356-.067-.525l5.287-3.086c.401.375.938.607 1.53.607a2.252 2.252 0 002.25-2.25 2.252 2.252 0 00-2.25-2.25 2.252 2.252 0 00-2.25 2.25c0 .18.026.357.068.525L6.03 7.358A2.233 2.233 0 004.5 6.75 2.252 2.252 0 002.25 9a2.252 2.252 0 002.25 2.25c.593 0 1.129-.232 1.53-.607l5.344 3.116c-.038.157-.06.322-.06.491a2.186 2.186 0 102.186-2.186z"/>
												</svg>
											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div className="col-12 col-md-6 col-lg-6 col-xl-4">
						<div className="_ftre_fd_dlvry2_crd">
							<div className="_ftre_fd_dlvry2_pic">
								<img src="assets/img/ftre_fd_dlvry4.svg" alt="" />
							</div>
							<div className="_ftre_fd_dlvry2_inr">
								<ul className="_ftre_fd_dlvry2_ul">
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
								</ul>
								<p className="_ftre_fd_dlvry2_crd_titl">
									Indian Cusine Briyani Buffet
								</p>
								<p className="_ftre_fd_dlvry2_crd_txt">
									Ut enim ad minim veniam, quis nostrud exercitation.
								</p>
								<div className="_ftre_fd_dlvry2_btm">
									<div className="_ftre_fd_dlvry2_btm_lft">
										<p className="_ftre_fd_dlvry2_btm_txt">
											$5.10
										</p>
									</div>
									<div className="_ftre_fd_dlvry2_btm_r8">
										<ul className="_ftre_fd_dlvry2_btm_ul">
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" fill="none" viewBox="0 0 16 14">
												  <path stroke="#fff" strokeWidth="1.5" d="M7.414 2.175L8 2.907l.585-.732c.062-.077.103-.13.135-.174.05-.065.078-.103.128-.153a3.75 3.75 0 015.31 5.296L8 12.968 1.84 7.144a3.75 3.75 0 015.31-5.296c.05.05.08.088.128.153l.135.174z"/>
												</svg>
											</li>
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 18 18">
													<path fill="#fff" d="M13.5 12.064c-.57 0-1.084.221-1.474.577L6.683 9.525c.04-.169.067-.345.067-.525 0-.18-.026-.356-.067-.525l5.287-3.086c.401.375.938.607 1.53.607a2.252 2.252 0 002.25-2.25 2.252 2.252 0 00-2.25-2.25 2.252 2.252 0 00-2.25 2.25c0 .18.026.357.068.525L6.03 7.358A2.233 2.233 0 004.5 6.75 2.252 2.252 0 002.25 9a2.252 2.252 0 002.25 2.25c.593 0 1.129-.232 1.53-.607l5.344 3.116c-.038.157-.06.322-.06.491a2.186 2.186 0 102.186-2.186z"/>
												</svg>
											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div className="col-12 col-md-6 col-lg-6 col-xl-4">
						<div className="_ftre_fd_dlvry2_crd">
							<div className="_ftre_fd_dlvry2_pic">
								<img src="assets/img/ftre_fd_dlvry5.svg" alt="" />
							</div>
							<div className="_ftre_fd_dlvry2_inr">
								<ul className="_ftre_fd_dlvry2_ul">
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
								</ul>
								<p className="_ftre_fd_dlvry2_crd_titl">
									Chiken Tikka Buffalo Wing ..
								</p>
								<p className="_ftre_fd_dlvry2_crd_txt">
									Ut enim ad minim veniam, quis nostrud exercitation.
								</p>
								<div className="_ftre_fd_dlvry2_btm">
									<div className="_ftre_fd_dlvry2_btm_lft">
										<p className="_ftre_fd_dlvry2_btm_txt">
											$3.60
										</p>
									</div>
									<div className="_ftre_fd_dlvry2_btm_r8">
										<ul className="_ftre_fd_dlvry2_btm_ul">
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" fill="none" viewBox="0 0 16 14">
												  <path stroke="#fff" strokeWidth="1.5" d="M7.414 2.175L8 2.907l.585-.732c.062-.077.103-.13.135-.174.05-.065.078-.103.128-.153a3.75 3.75 0 015.31 5.296L8 12.968 1.84 7.144a3.75 3.75 0 015.31-5.296c.05.05.08.088.128.153l.135.174z"/>
												</svg>
											</li>
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 18 18">
													<path fill="#fff" d="M13.5 12.064c-.57 0-1.084.221-1.474.577L6.683 9.525c.04-.169.067-.345.067-.525 0-.18-.026-.356-.067-.525l5.287-3.086c.401.375.938.607 1.53.607a2.252 2.252 0 002.25-2.25 2.252 2.252 0 00-2.25-2.25 2.252 2.252 0 00-2.25 2.25c0 .18.026.357.068.525L6.03 7.358A2.233 2.233 0 004.5 6.75 2.252 2.252 0 002.25 9a2.252 2.252 0 002.25 2.25c.593 0 1.129-.232 1.53-.607l5.344 3.116c-.038.157-.06.322-.06.491a2.186 2.186 0 102.186-2.186z"/>
												</svg>
											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div className="col-12 col-md-6 col-lg-6 col-xl-4">
						<div className="_ftre_fd_dlvry2_crd">
							<div className="_ftre_fd_dlvry2_pic">
								<img src="assets/img/ftre_fd_dlvry6.svg" alt="" />
							</div>
							<div className="_ftre_fd_dlvry2_inr">
								<ul className="_ftre_fd_dlvry2_ul">
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
									<li>
										<svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" fill="none" viewBox="0 0 18 17">
										  <path fill="#FEB618" d="M8.049.927c.3-.921 1.603-.921 1.902 0l1.294 3.983a1 1 0 00.951.69h4.188c.969 0 1.371 1.24.588 1.81L13.584 9.87a1 1 0 00-.364 1.118l1.295 3.983c.299.921-.756 1.688-1.54 1.118L9.589 13.63a1 1 0 00-1.176 0L5.024 16.09c-.783.57-1.838-.197-1.539-1.118L4.78 10.99a1 1 0 00-.363-1.118L1.028 7.41c-.783-.57-.38-1.81.588-1.81h4.188a1 1 0 00.95-.69L8.05.927z"/>
										</svg>
									</li>
								</ul>
								<p className="_ftre_fd_dlvry2_crd_titl">
									Fsish Fry Shashik Freid Fish ..
								</p>
								<p className="_ftre_fd_dlvry2_crd_txt">
									Ut enim ad minim veniam, quis nostrud exercitation.
								</p>
								<div className="_ftre_fd_dlvry2_btm">
									<div className="_ftre_fd_dlvry2_btm_lft">
										<p className="_ftre_fd_dlvry2_btm_txt">
											$6.40
										</p>
									</div>
									<div className="_ftre_fd_dlvry2_btm_r8">
										<ul className="_ftre_fd_dlvry2_btm_ul">
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" fill="none" viewBox="0 0 16 14">
												  <path stroke="#fff" strokeWidth="1.5" d="M7.414 2.175L8 2.907l.585-.732c.062-.077.103-.13.135-.174.05-.065.078-.103.128-.153a3.75 3.75 0 015.31 5.296L8 12.968 1.84 7.144a3.75 3.75 0 015.31-5.296c.05.05.08.088.128.153l.135.174z"/>
												</svg>
											</li>
											<li>
												<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 18 18">
													<path fill="#fff" d="M13.5 12.064c-.57 0-1.084.221-1.474.577L6.683 9.525c.04-.169.067-.345.067-.525 0-.18-.026-.356-.067-.525l5.287-3.086c.401.375.938.607 1.53.607a2.252 2.252 0 002.25-2.25 2.252 2.252 0 00-2.25-2.25 2.252 2.252 0 00-2.25 2.25c0 .18.026.357.068.525L6.03 7.358A2.233 2.233 0 004.5 6.75 2.252 2.252 0 002.25 9a2.252 2.252 0 002.25 2.25c.593 0 1.129-.232 1.53-.607l5.344 3.116c-.038.157-.06.322-.06.491a2.186 2.186 0 102.186-2.186z"/>
												</svg>
											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
				<div className="_ftre_fd_dlvry2_btn_all">
					<button className="_ftre_fd_dlvry2_btn">
						See All Menu
						<svg xmlns="http://www.w3.org/2000/svg" width="9" height="14" fill="none" viewBox="0 0 9 14">
						  <path fill="#36BA58" d="M0 12.35L5.565 7 0 1.65 1.717 0 9 7l-7.283 7L0 12.35z"/>
						</svg>
					</button>
				</div>
			</div>
		</section>
        </>
    );
};

export default Foods;